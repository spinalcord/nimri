import std/[algorithm, json, macros, math, options, os, sets, strutils, tables]
import mustache
import webui

type
  FrontendCommand = proc(args: JsonNode): JsonNode {.closure.}
...
macro collectFrontendBinding*(target: typed; modulePath, nimName,
  ...
  binding.returnType = frontendType(formalParams[0])
...
macro accessible*(definition: untyped): untyped =
  ## Exposes a synchronous procedure through the frontend command registry and
      ...
      if parameter[^1].kind == nnkEmpty:
        adapterBody.add quote do:
          ...
          let `localValue` = frontendFromJson(
            `argsNode`[`parameterNameLiteral`], `parameterType`)
      else:
        let defaultValue =
  ...
  let
    nimNameLiteral = newLit(nimName)
    modulePathLiteral = newLit(modulePath)
    wireNameLiteral = newLit(wireName)
  result = newStmtList(definition)
  result.add newCall(bindSym"collectFrontendBinding", target,
    modulePathLiteral, nimNameLiteral, wireNameLiteral)
  result.add quote do:
    registerFrontendCommand(
      `wireNameLiteral`,
...
proc dispatchFrontendRequest*(requestJson: string): string =
  ...
  try:
    ...
    let commandName = request["command"].getStr()
    ...
    let value = frontendCommands[commandName](args)
...
proc bindFrontendCommands*(window: Window) =
  ...
  window.bind("__invoke", proc(event: Event): string =
    dispatchFrontendRequest(event.getString())
  )
